...
int isNonZero (sparseterm *s) {		// checks if a matrix is null matrix
	return s[0].value > 0;
}

int compare (sparseterm A, sparseterm B, int col) { 	// compare positions of two terms to store in row major order
...
void storeSum (sparseterm* C, int* t, int r, int c, int v) {		// attach a term to the resultant matrix
